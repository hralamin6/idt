...
">
        ...
        <div class="py-4 flex flex-col justify-start md:w-1/2 m-auto">
            @foreach($items as $i => $item)
                <div @if(!$is_single_page) x-cloak x-show="step=={{$i+1}}"
                    ...
                    <legend class="text-lg font-medium my-1"><span>({{$i+1}})</span> What is the atomic mass of <span
                            class="text-primary">{{$item->symbol}}?</span></legend>
                    @if(!$is_mcq)
                        <input type="number" placeholder="Atomic mass, e.g: 02" x-model="ans[{{$i}}]"
                               class="input input-bordered input-info focus:shadow-none focus:outline-none input-sm max-w-xs">
                    @else
                        <ul class="grid grid-cols-2 gap-4">
                            @foreach($item->int_options($item, 'atomic_mass') as $j => $option)
                                <li>
                                    <label class="flex items-center text-sm">
                                        <input x-model="ans[{{$i}}]" value="{{number_format($option->atomic_mass)}}" type="radio"
                                               class="w-4 h-4 border border-gray-300 rounded-md"/>
                                        <span class="ml-3 text-md font-medium">{{number_format($option->atomic_mass)}}</span>
                                    </label>
                                </li>
                            @endforeach
                        </ul>
    ...
    @else
        <div class="py-8 flex flex-col justify-start md:w-1/2 m-auto capitalize">
            @foreach($items as $i => $item)
                <div
                    class="border border-2 rounded-lg border-purple-400 p-3 my-2 {{strtolower($ans[$i])==strtolower(number_format($item->atomic_mass))?'bg-green-100':'bg-red-100'}} ">
                    <legend class="text-lg font-medium my-1"><span>({{$i+1}})</span> What is the atomic mass of <span
                            class="text-primary">{{$item->symbol}}?</span></legend>
                    <ul class="grid grid-cols-2 gap-4">
                        <li>
                            <label class="flex items-center text-sm">
                                    <span class="ml-3 text-md font-medium">Your ans:
                                        <span
                                            class="{{strtolower($ans[$i])==strtolower(number_format($item->atomic_mass))?'text-blue-600':'text-red-600'}}">{{$ans[$i]?$ans[$i]:'no answer'}}</span>
                                    </span>
                            </label>
                        </li>
                        <li>
                            <label class="flex items-center text-sm">
                                    <span class="ml-3 text-md font-medium">Correct ans:
                                        <span class="text-green-600">{{number_format($item->atomic_mass)}}</span>
                                    </span>
                            </label>
                        </li>
                    </ul>
            ...
            @endforeach
            <center>
                <a href="{{route('practise.symbol.mass')}}" class="btn btn-outline btn-primary btn-xs btn-block w-48">Try
                    again</a>
            </center>
        </div>
    @endif
