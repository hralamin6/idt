...
">
        ...
        <div class="py-4 flex flex-col justify-start md:w-1/2 m-auto">
            @foreach($items as $i => $item)
                <div @if(!$is_single_page) x-cloak x-show="step=={{$i+1}}"
                    ...
                    <legend class="text-lg font-medium my-1"><span>({{$i+1}})</span> What is the atomic group of <span
                            class="text-primary">{{$item->symbol}}?</span></legend>
                    @if(!$is_mcq)
                        <input type="number" placeholder="Atomic group, e.g: 02" x-model="ans[{{$i}}]"
                               class="input input-bordered input-info focus:shadow-none focus:outline-none input-sm max-w-xs">
                    @else
                        <ul class="grid grid-cols-2 gap-4">
                            @foreach($item->pos_options($item, 'ypos') as $j => $option)
                                <li>
                                    <label class="flex items-center text-sm">
                                        <input x-model="ans[{{$i}}]" value="{{$option->ypos}}" type="radio"
                                               class="w-4 h-4 border border-gray-300 rounded-md"/>
                                        <span class="ml-3 text-md font-medium">{{$option->ypos}}</span>
                                    </label>
                                </li>
                            @endforeach
                        </ul>
    ...
    @else
        <div class="py-8 flex flex-col justify-start md:w-1/2 m-auto capitalize">
            @foreach($items as $i => $item)
                <div
                    class="border border-2 rounded-lg border-purple-400 p-3 my-2 {{strtolower($ans[$i])==strtolower($item->ypos)?'bg-green-100':'bg-red-100'}} ">
                    <legend class="text-lg font-medium my-1"><span>({{$i+1}})</span> What is the atomic group of <span
                            class="text-primary">{{$item->symbol}}?</span></legend>
                    <ul class="grid grid-cols-2 gap-4">
                        <li>
                            <label class="flex items-center text-sm">
                                    <span class="ml-3 text-md font-medium">Your ans:
                                        <span
                                            class="{{strtolower($ans[$i])==strtolower($item->ypos)?'text-blue-600':'text-red-600'}}">{{$ans[$i]?$ans[$i]:'no answer'}}</span>
                                    </span>
                            </label>
                        </li>
                        <li>
                            <label class="flex items-center text-sm">
                                    <span class="ml-3 text-md font-medium">Correct ans:
                                        <span class="text-green-600">{{$item->ypos}}</span>
                                    </span>
                            </label>
                        </li>
                    </ul>
            ...
            @endforeach
            <center>
                <a href="{{route('practise.symbol.group')}}" class="btn btn-outline btn-primary btn-xs btn-block w-48">Try
                    again</a>
            </center>
        </div>
    @endif
