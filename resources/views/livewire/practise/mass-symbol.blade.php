...
">
        ...
        <div class="py-4 flex flex-col justify-start md:w-1/2 m-auto">
            @foreach($items as $i => $item)
                <div @if(!$is_single_page) x-cloak x-show="step=={{$i+1}}"
                    ...
                    <legend class="text-lg font-medium my-1"><span>({{$i+1}})</span> atomic mass  <span
                            class="text-primary">{{number_format($item->atomic_mass)}}</span><span> for which atom?</span> </legend>
                    @if(!$is_mcq)
    ...
    @else
        <div class="py-8 flex flex-col justify-start md:w-1/2 m-auto capitalize">
            ...
            @endforeach
            <center>
                <a href="{{route('practise.mass.symbol')}}" class="btn btn-outline btn-primary btn-xs btn-block w-48">Try
                    again</a>
            </center>
        </div>
    @endif
