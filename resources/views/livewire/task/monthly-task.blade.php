                    ...
                    <tr>
                        ...
                        @foreach($items as $i => $item)
                            <th class="px-1 w-36 text-purple-800 dark:text-purple-400 border dark:border-indigo-400">
                                <div @mouseenter="hover = {{$item->id}}" @mouseleave="hover = 0" class="flex flex-col mx-auto items-center justify-center">
                                    <span>{{$lang==='en'?$item->name:$item->name_bn}}</span>
                                    <center>
                                    ...
                                    <div
                                        ...
                                        <span class="inline-block text-xs leading-tight">{!! $lang==='en'?$item->description:$item->description_bn !!}</span>
                                    </div>
                                </div>
                            </th>
                        ...
                        <tr class="">
                            ...
                            @foreach($items as $i => $item)
                                <td class="py-1 font-medium text-gray-900 border dark:border-indigo-400"> @if($task!=null) @if(in_array($item->id, @$task->tasks))
                                       <span class="font-bold text-blue-600">&#10004</span>  @else &#10060 @endif @endif </td>
                            @endforeach
                            <td class="py-1 font-medium text-gray-900 border dark:text-gray-200 dark:border-indigo-400">{{@$point->points}}</td>
                        </tr>
                    @endforeach
                    <tr>
