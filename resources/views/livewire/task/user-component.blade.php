        ...
        <li class="border-gray-400 flex flex-row mb-2 capitalize">
            <div class="shadow border select-none cursor-pointer bg-white dark:bg-gray-800 rounded-md flex flex-1 space-x-3 items-center p-4">
                ...
                <div class="flex flex-col gap-2">
                    <div class="text-green-600 dark:text-green-200 text-sm">@lang('total point:') {{$user->point()}}</div>
                    <div class="text-pink-600 dark:text-pink-200 text-xs">@lang('average point:') {{number_format($user->avgpoint(), 2)}}</div>
                </div>
            </div>
        </li>
        @endforeach
