<div class="container mx-auto grid" x-data="{atoms:  $persist(@js($atms)), rows: @entangle('selectedRows').defer, selectPage: @entangle('selectPageRows')}">
...
{{--                </tbody>--}}
                <tbody
                    class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800"
                >
                <template x-for="(atom, i) in atoms">
                        <tr :key="atom.id" class="text-gray-700 dark:text-gray-300 capitalize"
{{--                            :class="{'bg-gray-200 dark:bg-gray-600': rows.includes('{{$atom->id}}') }"--}}
                        >
{{--                            <td class="px-4 py-3">--}}
{{--                                <input x-model="rows" class="ring-none dark:bg-gray-400" type="checkbox" value="{{ $atom->id }}" name="todo2" id="{{ $atom->id }}">--}}
{{--                            </td>--}}
{{--                            <td class="px-4 py-3" x-text="i.current_page"></td>--}}
{{--                            <td class="px-4 py-3" x-text="atom.name"></td>--}}
                            <td class="px-4 py-3" x-text="i+1"></td>
                            <td class="px-4 py-3" x-text="atom.number"></td>
                            <td class="px-4 py-3" x-text="atom.symbol"></td>
                            <td class="px-4 py-3" x-text="atom.name"></td>
                            <td class="px-4 py-3" x-text="atom.atomic_mass"></td>
