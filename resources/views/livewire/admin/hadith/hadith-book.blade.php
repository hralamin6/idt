...
"
>
    <aside class="border dark:border-gray-600 row-span-4 bg-white dark:bg-darkSidebar" x-data="{rows: @entangle('selected_rows').defer, selectPage: @entangle('select_page_rows')}">
                ...
                <div class="w-full overflow-x-auto">
                    <table class="w-full whitespace-no-wrap">
                        ...
                        >
                            ...
                            <th class="px-4 py-3">@lang('name en')</th>
                        ...
                        @forelse($items as $i => $item)
                            <tr id="item-id-{{$item->id}}" class="text-gray-700 dark:text-gray-300 capitalize" :class="{'bg-gray-200 dark:bg-gray-600': rows.includes('{{$item->id}}') }">
                                ...
                                <td class="px-4 py-3 text-sm"><a href="{{route('admin.hadith.chapter', $item)}}">{!! $item->name_en !!}</a> </td>
                                <td class="px-4 py-3 text-sm">{{$item->name_bn}}</td>
                                <td class="px-4 py-3 text-xs">
                                <span wire:click.prevent="changeStatus({{$item->id}})" class=" cursor-pointer px-2 py-1 font-semibold rounded-full {{ $item->is_active? 'bg-green-300 dark:bg-green-700': 'bg-red-300 dark:bg-red-700' }} ">
            ...
            <div @click.outside="modal= false, editMode = false" class="container mx-auto w-11/12 md:w-2/3 max-w-lg ">
                <form @submit.prevent="editMode? $wire.editData(): $wire.saveData()" class="relative py-3 px-5 md:px-10 bg-white dark:bg-darkSidebar shadow-md rounded border border-gray-400 dark:border-gray-600 capitalize">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-2">
                        ...
                        <div>
                            <label for="name" class="text-gray-800 dark:text-gray-200 text-sm font-bold leading-tight tracking-normal">@lang('name en')</label>
                            <input wire:model.lazy="state.name_en" class="input input-bordered input-info w-full input-sm max-w-xs"/>
                            @error('name_en')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="book_key" class="text-gray-800 dark:text-gray-200 text-sm font-bold leading-tight tracking-normal">@lang('book key')</label>
                            <input wire:model.lazy="state.book_key" class="input input-bordered input-info w-full input-sm max-w-xs"/>
                            @error('book_key')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label for="section_number" class="text-gray-800 dark:text-gray-200 text-sm font-bold leading-tight tracking-normal">@lang('section number')</label>
                            <input wire:model.lazy="state.section_number" class="input input-bordered input-info w-full input-sm max-w-xs"/>
                            @error('section_number')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div>
                        ...
                        <div>
                            <label for="priority" class="text-gray-800 dark:text-gray-200 text-sm font-bold leading-tight tracking-normal">@lang('priority')</label>
                            <input wire:model.lazy="state.priority" class="input input-bordered input-info w-full input-sm max-w-xs"/>
                            @error('priority')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>
                        <div class="form-control">
                            <label  class="label cursor-pointer">
                                <span class="label-text">@lang('status')</span>
                                <input wire:model.lazy="state.is_active" type="checkbox" checked="checked" class="checkbox checkbox-primary">
                            </label>
